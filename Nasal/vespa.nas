...
var loop = func {
	...
	if ((gear.getValue() > 1) and (rpm.getValue() < 200) and (clutch.getValue() == 0)) {
	 running.setValue(0);
	}
	...
	if ((cranking.getValue() == 1) and ((gear.getValue() == 0) or (clutch.getValue == 1)) and (fuel.getValue()>=0.00000009)) {
	 running.setValue(1);	 
	 cranking.setValue(0);
	}
	...
	if (running.getValue() == 1) {
	 if (fuel.getValue() < 0.00000009) {
	  running.setValue(0);
	  }
	 else {
	  fuel_lev = fuel.getValue();
	  fuel.setValue(fuel_lev - (0.9*elevator.getValue()+0.1)*0.00000009);
	 }
	}
...
loop();
aircraft.livery.init("Aircraft/Vespa-GS125/Models/Liveries", "sim/model/livery/name");
